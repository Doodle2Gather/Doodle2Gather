...
final class WebSocketController: ObservableObject {
  @Published var actions: [UUID: NewDoodleActionResponse]
  ...
  func addAction(_ content: String) {
    ...
    let message = NewDoodleActionMessage(id: id, content: content)
    do {
  ...
  func handleQuestionResponse(_ data: Data) throws {
    // 1
    let response = try decoder.decode(NewDoodleActionResponse.self, from: data)
